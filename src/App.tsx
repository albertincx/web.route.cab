...
const AddRouteModal: React.FC<{
...
}> = ({show, form: _form, onClose, onSubmit, editRoute, handleClose, renderErrorMessage, handleChange, openLocationPicker}) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!form.name || !form.time || !form.contact || form.days.length === 0) {
...
function App() {
    ...
    const handleAddRoute = (newRoute: Partial<Route>) => {
        if (editingRoute) {
            ...
            sendNewRouteToServer(newRoute as Route).then(() => {
                // @ts-ignore
